...
public class SliceConditionTyped<T extends ISignatureType> implements ISliceSinglePositionTyped<T> {
    ...
    public SliceConditionTyped() {
	super();
	initiateIfNull(null, null, null);
    }

    public SliceConditionTyped(MDElement pos, String cond, String uri, SliceConditionStatus status) {
	this.condition = cond;
    ...
    public SliceConditionTyped(MDElement pos, String cond, String uri, String type, SliceConditionStatus status) {
	this.condition = cond;
	this.positionOfCondition = pos;
	this.uri = uri;
	this.type = type;
    ...
    public SliceConditionTyped(Signature<T> signature) {
	this.signature = signature;
    }

    public SliceConditionTyped(Signature<T> signature, String type) {
	this(signature);
	this.setType(type);
    }

    public SliceConditionTyped(MDElement pos, String cond, String uri, Signature<T> signature,
	    SliceConditionStatus status) {
	this(pos, cond, uri, status);
	this.signature = signature;
    }

    public SliceConditionTyped(MDElement pos, String cond, String uri, String type, Signature<T> signature,
	    SliceConditionStatus status) {
     ...
     * 
     */
    private SliceConditionTyped(SliceConditionTyped<T> pred) {
	...
	if (o instanceof SliceConditionTyped) {
	    SliceConditionTyped<T> sc = (SliceConditionTyped<T>) o;
	...
	SliceConditionTyped<T> sc = (SliceConditionTyped<T>) sourceVar;
	...
	SliceConditionTyped<T> sc = (SliceConditionTyped<T>) tempVariable;
	this.setCondition(sc.getCondition());
	...
	// Type information should not be removed upon unbinding
	// this.setType("");
	this.setURI(null);
	...
	return new SliceConditionTyped<T>(this);
    }
	...
	return new SliceConditionTyped<T>(getPositionOfCondition(), getCondition(), getURI(), getType(),
