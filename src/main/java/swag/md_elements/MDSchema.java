...
public interface MDSchema extends Graph<MDElement, MDRelation> {
  ...
  /**
   * 
   * Gets all MD elements leading to the element specified by the passed {@code uri}.
   * 
   * @param uri
   * @return
   * 
  ...
  /**
   * 
   * Adds an MD element along with its MD relations to the graph (a new entry of key value pair).
   * 
  ...
  /**
   * 
   * Removes an MD element along with its MD relations from the graph (remove an entry of key value
   * pair).
   * 
  ...
  /**
   * 
   * Copies an MD element along with its MD relations to the graph (a copy entry of key value pair).
   * 
  ...
  /**
   * 
   * URI may not be unique, only identifying name is unique. This function gets nodes that share the
   * same URI.
   * 
   * @return
   * 
   ...
   * @return
   */
  public String getIdentifyingNameFromUriAndDimensionAndHier(String elemUri, String dimURI,
      String hierURI);
   ...
   * @return
   */
  public default Set<MDElement> getAllElementsOnHierarchyInDimension(String hierUri,
      String dimensionURI) {
  ...
  /**
   * 
   * Returns all elements that are either on a hierarchy in dimension or all base measures or all
   * aggregated measures.
   * 
   * @param hierUri
   * @param dimensionURI
  ...
  public default Set<Level> getAllLevelsOnHierarchyInDimension(String hierUri,
      String dimensionURI) {
  ...
  public default HierarchyInDimension getHierarchyInDimensionNode(String dimStr, String hStr) {
    ...
    return getAllNodes()
        .stream()
        .filter(x -> (x instanceof HierarchyInDimension)
            && x.getURI().equals(h.getURI() + dim.getURI()))
        .map(x -> (HierarchyInDimension) x)
        .findFirst()
        .orElse(null);
    //@formatter:on
  }
   ...
   * @return
   */
  public default Level getNextLevelInHierarchy(String currentLevelName, String hierUri,
      String dimensionURI) {
    for (Level l : getAllLevelsOnHierarchyInDimension(hierUri, dimensionURI)) {
      for (MDRelation rel : getEdgesOfNode(l)) {
        if (rel instanceof QB4OHierarchyStep) {
          if ((getAllLevelsOnHierarchyInDimension(hierUri, dimensionURI))
              .contains(rel.getTarget())) {
   ...
   * 
   * @return true if level1 rolls up (directly or indirectly) to level2, false otherwise.
   * 
   */
  ...
  /**
   * 
   * Checks if item is upper in dimension than level, i.e., item is either a non dimensional
   * attribute of level or level rolls up to item.
   * 
   * @param level
   * @param item
   * 
   * @return true if item is either a non-dimensional attribute of level or level rolls up to item;
   *         false otherwise
   * 
   */
   ...
   * @param item
   * 
   * @return 2: level either rolls up to item or item is a descriptor of level. 1: level and item
   *         are equal 0: item rolls up to level. -1: no common path; item and level are
   *         incomparable
   * 
   */
  ...
  /**
   * 
   * returns the paths between two nodes, each path is a sequence of edges. ALl the edges in the
   * paths are edges that can have a mapping. never exploring a path twice is guaranteed on the node
   * level; each node's out edges are explored once
   * 
  ...
  public default Set<MDElement> getNodesByIdentifyingNameSimilarity(String name) {
    return getAllNodes().stream().filter(x -> x.getIdentifyingName().contains(name))
        .collect(Collectors.toSet());
  }

  @Override
  public default Set<MDElement> getNodesByName(String name) {
    return getAllNodes().stream().filter(x -> x.getName().contains(name))
        .collect(Collectors.toSet());
  }
   ...
   * 
   * @return the hierarchy of level if exists. The default hierarchy if no hierarchy exists for
   *         level. null if the level is not found.
   * 
   */
