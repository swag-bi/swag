...
public class BasicAnalysisSituationSPARQLQueryGeneratorQB4ODiss extends BasicAnalysisSituationSPARQLQueryGenerator {

	public BasicAnalysisSituationSPARQLQueryGeneratorQB4ODiss(QueryUtils utils, MDSchemaGraphQB4OLAP schema) {
		super();
		this.utils = utils;
		this.schema = schema;
	}

	private static final Logger logger = Logger.getLogger(BasicAnalysisSituationSPARQLQueryGeneratorQB4ODiss.class);
	private TripleCollectorBGP bgp = new TripleCollectorBGP();
	private QueryUtils utils;
	private MDSchemaGraphQB4OLAP schema;

	/**
	 * Generates a query from a dimensionToAS This function is unaware of the
	 * variables, no special treatments for variables
	 * 
	 * @param dimToAS dimension to analysis situation at hand
	 * @return a rooted query for the dimensionToAS at hand
	 */
	public CustomSPARQLQuery generateDimQuery(IDimensionQualification dimToAS) {
	 ...
	 * @param varName the name of the variable on which to apply a FILTER expression
